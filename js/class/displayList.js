 class DisplayList {
    ...
    createIngredientsList(){
       ...
       ulIngredient.innerHTML=`${this.ingredients && this.ingredients.map((ingredient) => `<li class='itemLi'>${ingredient}</li>`).join("")}`;
     
    }

    createApplianceList(){
        const ulAppliance = document.querySelector('#dropdown-search-list-appareil')
        ulAppliance.innerHTML=`${this.appliances && this.appliances.map((appliance) => `<li class='itemLi'>${appliance}</li>`).join("")}`;
        
    }

    createUstensilsList(){
        const ulUstensil = document.querySelector('#dropdown-search-list-ustensiles')
        ulUstensil.innerHTML=`${this.ustensils && this.ustensils.map((ustensil)=> `<li class='itemLi'>${ustensil}</li>` ).join("")}`;
